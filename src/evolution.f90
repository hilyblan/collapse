...
contains
  ...
  subroutine evolution_chemistry
    ...
    if (debug) then
       ...
       write(*,*) "grmin, grmax rho_grain ratio_grain_gas r_gr_scale MD_grain R_grain Dens_GRAIN rgrain2 rgrain3 r_grain:"
       write(*,*) grmin,grmax,rho_grain,ratio_grain_gas,r_gr_scale,&
    ...
    do i=1,nreact
       ...
       CASE ('CRIO_DIR','CRIO_IND')
          ...
          !    if ((ir1.eq.ind_ggminus).or.(ir1.eq.ind_gg0)) then
       ...
       CASE ('H2_FO')
          ...
          ! R-factor: formation of h2 on grains
          if (ip1.eq.ind_oh2.or.ip1.eq.ind_ph2) r_gr_h2   = creation_rate/densityR2/nh_init
...
!!$    if (debug) &
!!$         write(*,'("D-diffun: Htot, H, oH2, pH2, H2tot, H+, e-,n,i,neg:",(13es12.3))') &
!!$         nh,dens_h,dens_oh2,dens_ph2,dens_h2,dens_hplus,dens_e,densityn,densityi,densityneg
    !
    ...
    md_grain = dot_product(yarray(bv_cor:ev_cor),dble(speci(b_cor:e_cor)%mass))
    md_grain = md_grain+dot_product(yarray(bv_gra:ev_gra),dble(speci(b_gra:e_gra)%mass))
    vd_grain = dot_product(yarray(bv_cor:ev_cor),dble(speci(b_cor:e_cor)%mass)) / rho_grain &
         + dot_product(yarray(bv_gra:ev_gra),dble(speci(b_gra:e_gra)%mass)) / rho_mantle
    rgrain3 = 3.0_dp*vd_grain/(4.*pi*dens_grain)
    rgrain2 = r_gr_scale*rgrain3**(2./3.)
...
!!$    dy(iv_xsphere) = -0.5d0*pi*xsphere*(1.-xsphere)**0.5d0/tau_collapse/vn
!    dy(iv_xsphere) = -0.5d0*pi*(1./xsphere-1.)**0.5d0/tau_collapse/vn
!    dy(iv_xsphere) = -0.5d0*pi*(-log(xsphere))**0.5d0/tau_collapse/vn
    dy(iv_xsphere) = -1.d0/maxdist
    vn = 0.5d0*maxdist*pi*(-log(xsphere))**0.5d0/tau_collapse
    yarray(iv_vn) = vn
    vi = vn
    !
    !
...
!       dy(iv_vn) = -pi**2/8.d0*maxdist/tau_collapse**2/xsphere/vn
       dy(iv_vn) = 0.0_dp
    endif
    !
    ...
    !
    ! Mass densities
    dilution      = 2*dy(iv_xsphere)/xsphere ! for Larson-Penston
    ...
    ! Grain coagulation
    !
    ! gradient of grain number density, assuming a unique radius;
    ! varies because of grain coagulation
    ! dn_g/dt = -4pi*Rgrain2*n_g^2*v_rel
    !
    ! v_rel is the collision speed, integrated over a double Gaussian
    ! distribution, each of width (FWHM) v_turb, the turbulent
    ! velocity [of the order of the sound speed, c_s].
    !
    ! v_rel = [2/(pi ln 2)]^0.5 Delta {1 - [1 + 2 ln 2 (v_crit/v_turb)^2] * exp[-2 ln 2 (v_crit/v_turb)^2]}
    !
    ! We take the critical velocity v_crit for coagulation from
    ! Chokshi et al. 1993, ApJ, 407, 806; Fig. 8
    !
    !   v_crit = 1.D-2 / r_grain**(5.d0/6.d0) for graphite;
    !   r_grain in cm, v_crit in cm s-1
    !   v_crit = 1.D-1 / r_grain**(5.d0/6.d0) for ice;
    !   Poppe & Blum, 1997, Adv. Sp. Res., 20, 1595
    !   v_crit = 4.D-1 / r_grain**(5.d0/6.d0) for ice;
    !   v_crit = 0 for no coagulation
    ! Note: v_rel = 0. if v_crit=0
    v_crit = 0d0
