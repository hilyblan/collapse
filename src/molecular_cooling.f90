module module_molecular_cooling
  ...
  !
  ! Contains all variables and subroutines needed to calculate the
  ! radiative cooling rates due to several molecules, but not H2 (see
  ! MODULE_H2).
  !
  ! This is a direct function of DY(iv_Vn), so this is really an
  ! implicit equation
  !
...
!!$  include "precision.f90"
  ...
  !
  ! File logical unit and file name for error messages ---
  integer(kind=4) :: f_err_cool
...
contains
  ...
  subroutine molecular_cooling_openfile
  ...
  end subroutine molecular_cooling_openfile

  subroutine compute_molecular_cooling
    use module_phys_var, only : tn, vgrad, cool_kn,yarray,bv_speci
    ...
    molec_cool        = zero
    !
    ! Molecular cooling is computed only if required
    if (.not.do_thermal) return
    !
    ! Initialize density for each species
    ! H, H2, oH2, pH2, H+, G0, G+, G-, and e- have been calculated in 'diffun'
    call get_density(yarray,ind_he,dens_he)
    call get_density(yarray,ind_o,dens_o)
    call get_density(yarray,ind_c,dens_c)
    call get_density(yarray,ind_n,dens_n)
    call get_density(yarray,ind_s,dens_s)
    call get_density(yarray,ind_h2o,dens_h2o)
    call get_density(yarray,ind_oh,dens_oh)
    call get_density(yarray,ind_co,dens_co)
    call get_density(yarray,ind_nh3,dens_nh3)
    call get_density(yarray,ind_si,dens_si)
    call get_density(yarray,ind_oplus,dens_oplus)
    call get_density(yarray,ind_cplus,dens_cplus)
    call get_density(yarray,ind_splus,dens_splus)
    call get_density(yarray,ind_nplus,dens_nplus)
    call get_density(yarray,ind_siplus,dens_siplus)
    call get_density(yarray,ind_feplus,dens_feplus)
    !
    !---------------------------------------
    ...
    !----------------------------------------
    ! "more precised" cooling for H2O and CO
